...
fh = logging.FileHandler('language_main_fedma.log')
...
def add_fit_args(parser):
    ...
    """
    # Training settings
    parser.add_argument('--mode', type=str, default='fedma', metavar='N',
...
if __name__ == "__main__":
    ...
    for client_index in range(n_clients):
        if retrain_flag:
            ...
            with open("trained_model_client_fedma_{}".format(client_index), 'wb') as trained_model_file:
                pickle.dump(model, trained_model_file)
        else:
            # load the trained model from local disk, and forward
            with open("trained_model_client_fedma_{}".format(client_index), 'rb') as trained_model_file:
